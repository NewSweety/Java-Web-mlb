...
public class SqlHelper {
	...
	public Manager queryManagerById(String manager_id){
		System.out.println(manager_id+"shujuku2");
		...
		System.out.println(manager+"shujuku");
		return manager;
	}
	...
	public List<Article> queryArticleByNav_id(String nav_id) {
		String sql="select article_id,article_title,article_content,article_data,nav_id from article where nav_id=? order by aticle_data desc";
	...
	public List<Article> queryArticleByNav_id4Index(String nav_id) {
		String sql="select article_id,article_title,article_data,nav_id from article where nav_id=? order by article_data desc limit 0,10";
		List <Article>list=new ArrayList();
		try{
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, nav_id);
	...
	public List<Map> queryArticleMapByPage(int p) {
		int start=p*perPage;
		/*String sql= "select article_id,article_title,article_data,nav_name "
		+ "from article,nav "
		+ "where article.nav_id = nav.nav_id "
		+ "order by article_data desc "
		+ "limit  "+start+","+perPage;*/
		String sql="select article.*,nav.* from article,nav where article.nav_id = nav.nav_id order by article_data desc limit "+start+","+perPage+"";
		List <Map>list=new ArrayList<Map>();
		try{
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
	...
	public List<Map> queryArticleMapByPage(int p,String nav_id,String article_title) {
		...
		+ "limit  "+start+","+perPage;*/
		String sql="select article.*,nav.* from article,nav where article.nav_id = nav.nav_id ";
		if(nav_id!=null&&!"".equals(nav_id)){
			sql+=" and nav.nav_id = ? ";
		}
		...
		sql+="order by article_data desc limit "+start+","+perPage+"";
